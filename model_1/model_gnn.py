...
def split_kn_train_test():
    df = pd.read_csv('all_data_chem.csv', header=0)
    df = shuffle(df)
    df_a = df[df['Label'] == 0]
    df_n = df[df['Label'] == 1]
...
def my_gnn_train_bi():
    ...
    train_gnn.test_classify(n_atom_feat=26, n_bond_feat=6, n_classes=2, test=test, classify_metrics=bi_classify_metrics, save_path='models/gnn.pth', device='cpu')
