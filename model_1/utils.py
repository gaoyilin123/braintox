...
def plot_confusion_matrix(cm, class_names, savename):
    ...
    ax.set_title('GNN Confusion Matrix (Percentage)', pad=20)
...
def bi_classify_metrics(labels, preds, plot_cm=False, savename='MLP', classnames=['Negative', 'Positive']):
    """
    example:
    labels = np.array([
                    [1,0], [1,0], [1,0], [0,1], [1,0],
    ...
    # 准确率
    q = (tp + tn) / (tp + fn + tn + fp + epsilon)

    # 马修斯相关系数
    mcc = (tp * tn - fn * fp) / (math.sqrt((tp + fn) * (tp + fp) * (tn + fn) * (tn + fp)) + epsilon)

    # 精确度
    P = tp / (tp + fp + epsilon)

    # F1得分
    F1 = (2 * P * se) / (P + se + epsilon)
    ...
    mcc = (tp * tn - fp * fn) / (
                (tp + fp) * (fn + tp) * (fn + tn) * (fp + tn)) ** 0.5  # equal: matthews_corrcoef(y_true, y_pred)
    ap = average_precision_score(y_true, y_pred)
    auc = roc_auc_score(y_true, y_pred)
    if plot_cm:
        plot_confusion_matrix(cm, classnames, save_path)
    return np.array([tp, tn, fp, fn, se, sp, mcc, q, P, f1, BA, auc])


def multi_classify_metrics(labels, preds, average_method='weighted', plot_auc_curve=False, plot_confuse_matrix=False,
                           savename="MLP", classnames=['', 'b', 'd', 'e', '']):
    """
    args:
